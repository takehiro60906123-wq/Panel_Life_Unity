...
public class BattleDebugToolEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.Space(8);
        EditorGUILayout.LabelField("─ 演出テスト ─", EditorStyles.miniLabel);

        // ステージクリア
        GUI.backgroundColor = new Color(0.4f, 1f, 0.7f);
        if (GUILayout.Button("F9  ステージクリア演出", GUILayout.Height(28)))
        {
            tool.TriggerStageClear();
        }

        // ゲームオーバー
        GUI.backgroundColor = new Color(0.6f, 0.2f, 0.2f);
        if (GUILayout.Button("F10  ゲームオーバー演出", GUILayout.Height(28)))
        {
            tool.TriggerGameOver();
        }
