...
namespace HeurekaGames.AssetHunterPRO
{
    public class AH_SettingsManager
    {
        ...
        private const int MinDependencyDepth = 1;
        private const int MaxDependencyDepth = 5;

        private const int RefCountRangeMin = 10;
        private const int RefCountRangeMax = 1000;
        ...
        private readonly static string PrefsDependencyDepth = "AH.DependencyDepth" + ProjectPostFix;
        private readonly static string PrefsMaxRefCount = "AH.MaxRefCount" + ProjectPostFix;    
        
        internal readonly static int InitialMaxRefCount = 200;
        internal readonly static int InitialDependencyDepth = 3;
        ...
        public int MaxRefCount
        {
            ...
            internal set { EditorPrefs.SetInt(PrefsMaxRefCount, value); }
        }
        ...
        public GUIContent[] GUIcontentignoredLists = new GUIContent[5]
     {
        ...
        internal void DrawSettings()
        {
            ...
            AutoCreateLog = drawToggle("Auto create log when building", AutoCreateLog, AH_SettingsManager.InitialValueAutoCreateLog);
            AutoOpenLog = drawToggle("Auto open log location after building", AutoOpenLog, AH_SettingsManager.InitialValueAutoOpenLog);
            AutoRefreshLog = drawToggle("Auto refresh when project changes", AutoRefreshLog, AH_SettingsManager.InitialValueAutoRefreshLog);
            EstimateAssetSize = drawToggle("Estimate runtime filesize for each asset", EstimateAssetSize, AH_SettingsManager.InitialValueEstimateAssetSize);
            HideButtonText = drawToggle("Hide buttontexts", HideButtonText, AH_SettingsManager.InitialValueHideButtonText);
            HideNewsButton = drawToggle("Hide 'News' button", HideNewsButton, AH_SettingsManager.InitialValueHideNewsButton);

            EditorGUILayout.BeginHorizontal();
            EditorGUI.BeginChangeCheck();
            IgnoreScriptFiles = drawToggle("Ignore script files", IgnoreScriptFiles, AH_SettingsManager.InitialIgnoreScriptFiles);
        ...
        internal void DrawDependencyGraphSettings()
        {
            EditorGUILayout.Space();

            EditorGUILayout.HelpBox("DependencyGraph Settings", MessageType.None);
            DependencyDepth = drawIntSlider("Max Dependency Depth", DependencyDepth, MinDependencyDepth, MaxDependencyDepth, AH_SettingsManager.InitialDependencyDepth);
            MaxRefCount = drawIntSlider("Reference Limit", MaxRefCount, RefCountRangeMin, RefCountRangeMax, AH_SettingsManager.InitialMaxRefCount);
        ...
        private bool drawToggle(string title, bool value, bool defaultVal)
        {
            return EditorGUILayout.ToggleLeft(title, value, (defaultVal != value) ? EditorStyles.boldLabel : EditorStyles.label);
        }

        private int drawIntSlider(string title, int value, int minValue, int maxValue, int defaultVal)
        {
            return EditorGUILayout.IntSlider(title, value, minValue, maxValue);

            /*var origFontStyle = EditorStyles.label.fontStyle;
            EditorStyles.label.fontStyle = (defaultVal != value) ? FontStyle.Bold : FontStyle.Normal;

            var curVal = EditorGUILayout.IntSlider(title, value, minValue, maxValue);
