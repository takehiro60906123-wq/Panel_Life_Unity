                ...
                if (dependencyGraphManager.HasSelection)
                {
                    ...
                    using (new EditorGUILayout.VerticalScope("box", GUILayout.ExpandWidth(true)))
                    {
                        using (new EditorGUILayout.HorizontalScope("box", GUILayout.ExpandWidth(true)))
                        {
                            ...
                            {
                                string isCappedMsg = ""; 
                                if (isValidFrom && viewFrom.treeModel.root.children.Count() == AH_SettingsManager.Instance.MaxRefCount)
                                {
                                    isCappedMsg = $"(Capped at {AH_SettingsManager.Instance.MaxRefCount})";
                                }
                                GUILayout.FlexibleSpace();
                                EditorGUILayout.LabelField($"A dependency of {(isValidFrom ? (viewFrom.treeModel.root.children.Count()) : 0)} {isCappedMsg}", EditorStyles.boldLabel);
                                GUILayout.FlexibleSpace();
                            }
                        }
                        if (isValidFrom)
                        ...
                        using (new EditorGUILayout.HorizontalScope("box", GUILayout.ExpandWidth(true)))
                        {
                            ...
                            {
                                string isCappedMsg = ""; 
                                if (isValidTo && viewTo.treeModel.root.children.Count() == AH_SettingsManager.Instance.MaxRefCount)
                                {
                                    isCappedMsg = $"(Capped at {AH_SettingsManager.Instance.MaxRefCount})";
                                }

                                GUILayout.FlexibleSpace();
                                EditorGUILayout.LabelField($"Depends on {(isValidTo ? (viewTo.treeModel.root.children.Count()) : 0)} {isCappedMsg}", EditorStyles.boldLabel);
                                GUILayout.FlexibleSpace();
                            }
                        }
                        if (isValidTo)
